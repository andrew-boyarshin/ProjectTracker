...
@app.get('/projects/<pr_id:int>/maintain')
@app.get('/projects/<pr_id:int>/maintain/')
def sow_dev(pr_id):
    ...
    return template2('andrew/stage4', data=data, pr_id=pr_id, ver=PROJECT_VERSION, date=str(datetime.now())[:-7],
                     cur_author=u.nickname)


@app.get('/projects/<pr_id:int>/maintain/load/<header_id:int>')
...
@app.get('/projects/<pr_id:int>/maintain/<ticket_id:int>/mark')
...
@app.get('/projects/<pr_id:int>/maintain/<ticket_id:int>/unmark')
...
@app.post('/projects/<pr_id:int>/maintain/<hid:int>/ticket_create')
def sow_dev_create_ticket(pr_id, hid):
    ...
    if len(data):
        ...
        Ticket.create(name=title, content=content,
                      ...
                      crit=False, closed=False, close_time='', nickname=nick, pid=pr_id)
        return 'OK'
    return 'FAIL'


@app.post('/projects/<pr_id:int>/maintain/ticket/<ticket_id:int>/set_category')
...
@app.post('/projects/<pr_id:int>/maintain/ticket/<ticket_id:int>/comment_create')
def sow_dev_create_comment(pr_id, ticket_id):
    ...
    if len(data):
        Comment.create(nick=nick, tid=ticket_id, msg=data, 
