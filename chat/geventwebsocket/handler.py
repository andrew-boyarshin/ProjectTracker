...
class WebSocketHandler(WSGIHandler):
    ...
    def upgrade_connection(self):
        ...
        if version_info.major == 3:
            secwebsockacc = str(base64.b64encode(
                hashlib.sha1(str(key).encode() + str(self.GUID).encode()).digest()))[2:-1]
        else:
            secwebsockacc = base64.b64encode(
                hashlib.sha1(key + self.GUID).digest())

        headers = [
            ("Upgrade", "websocket"),
            ("Connection", "Upgrade"),
            ("Sec-WebSocket-Accept", secwebsockacc),
        ]
