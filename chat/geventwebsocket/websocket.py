...
class WebSocket(object):
    ...
    def send_frame(self, message, opcode):
        ...
        try:
            if version_info.major == 3:
                ...
                self.raw_write(bytearray(header + message))
            else:
    ...
    def close(self, code=1000, message=''):
        ...
        try:
            ...
            self.send_frame(
                struct.pack('!H%ds' % len(message), code, message.encode()),
...
class Header(object):
    ...
    def encode_header(cls, fin, opcode, mask, length, flags):
        ...
        # print(first_byte, second_byte, extra)

        from sys import version_info
        if version_info.major == 3:
            return bytes([first_byte]) + bytes([second_byte]) + extra.encode()
        else:
